...
function Imgcomp() {
    ...
    const fadeone = useSpring({
        ...
        from: { opacity: 0 },
        config: isSmallScreen ? { duration: 0 } : { duration: 500 },
    });
    ...
    return (
        <div className={`transform transition-opacity duration-1000 ease-in-out ${loaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
            ...
            <section className='flex -mt-2  pt-12 pb-12 pl-2 pr-0'>
                <aside className="widt">
                    {isLoading ? (
                        <SkeletonLoader />
                    ) : (
                        <div className='sm:ml-5  lg:ml-0 xl:ml-0 xl:mr-0 lg:mr-0'>
                                <div className='grid grid-cols-2 xl:flex sm:grid-cols-1 xs:grid-cols-1 md:block -gap-2'>
                                <animated.div style={fadetwo}>{Returndlist[0]}</animated.div>
                                <animated.div style={fadeone}>{Returndlist[1]}</animated.div>
                            </div>
                                <div className="grid grid-cols-3 xl:flex sm:grid-cols-1 xs:grid-cols-1 md:block gap-1">
